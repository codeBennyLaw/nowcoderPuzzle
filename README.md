...
- 适配与性能：Pointer Events 统一触控与鼠标；GPU 加速拖拽；图片预解码；清单与音频预加载；本地缓存清单。
...
- 模式与难度：顶部控制栏选择模式（滑块固定 3×3），经典模式可调难度（3/4/5）。
...
- [js/script.js](js/script.js)：两种拼图模式逻辑、图片与 GIF 清单读取、统计与通关处理。
